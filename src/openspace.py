    ...
    def random_allocate(self, names):
        ...
        for n in range(len(names)):
            ...
            if df.isna().sum().sum() == 0:
                print(f'Sorry {names[n]}, there is no more seat availalbe here, pleasecheck yourself if the second room')
            else:
                while True:
                    ...
                    if pd.isna(random_spot) == True:
                        ...
                        print(f"Dear {names[n]}, please sit on table {random_column + 1} at place {random_row + 1}")
